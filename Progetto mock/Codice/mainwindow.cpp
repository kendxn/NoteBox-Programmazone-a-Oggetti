...
void MainWindow::on_actionSalva_triggered()
{
    ...
    QJsonArray arrayEtichette;
    for (const QString &et : elencoEtichette) {
        arrayEtichette.append(et);
    }
    ...
    rootObject["etichette"] = arrayEtichette;
    rootObject["note"] = arrayNote;

    QJsonDocument doc(rootObject); // Passiamo l'oggetto radice al documento
    QFile file(fileName);

    if (file.open(QIODevice::WriteOnly)) {
        file.write(doc.toJson(QJsonDocument::Indented));
        file.close();
        QMessageBox::information(this, "Salvataggio", "Dati e etichette salvati correttamente!");
    } else {
...
void MainWindow::on_actionCarica_triggered() {
    ...
    // --- RIPRISTINO ELENCO ETICHETTE ---
    // Puliamo la vecchia grafica dei widget etichetta nella sidebar
    QLayoutItem *childLabel;
    if (ui->frameLabel->layout()) {
        while ((childLabel = ui->frameLabel->layout()->takeAt(0)) != nullptr) {
            if (childLabel->widget()) childLabel->widget()->deleteLater();
            delete childLabel;
        }
    }

    elencoEtichette.clear();
    elencoEtichette.append("Generico"); // L'etichetta di base deve esserci sempre

    if (rootObject.contains("etichette")) {
        QJsonArray arrayEtichette = rootObject["etichette"].toArray();
        for (const QJsonValue &value : arrayEtichette) {
            QString et = value.toString();
            if (et != "Generico" && !elencoEtichette.contains(et)) {
                elencoEtichette.append(et);
                creaWidgetEtichetta(et); // Rigenera il widget con i tasti "Mostra" e "X"
            }
        }
    }
    aggiornaMenuEtichette(); // Sincronizza la ComboBox dei suggerimenti

    // --- RIPRISTINO POLIMORFICO DELLE NOTE ---
    qDeleteAll(listaNote);
    listaNote.clear();

    QJsonArray arrayNote = rootObject["note"].toArray();
    ...
    for (const QJsonValue &value : arrayNote) {
        ...
        if (n) {
            ...
            if (et.isEmpty() || !elencoEtichette.contains(et)) et = "Generico";
            n->etichetta = et;
    ...
    riposizionaCards();
    QMessageBox::information(this, "Successo", "Note e etichette caricate correttamente!");
}
...
void MainWindow::on_buttonSalva_clicked()
{
    ...
    QDateTime dataScadenzaIniziale = haScadenza ? ui->dateEdit->dateTime() : QDateTime();
    ...
    if (counter > 1) {
        ...
        na->setScadenza(dataScadenzaIniziale);
        nuovanota = na;
    }
    // gestione singoli casi: task, promemoria e immagine altrimenti nota generica
        ...
        promemoria* p = new promemoria();
        p->setScadenza(dataScadenzaIniziale);
        nuovanota = p;
    }
    else if (haImmagine) {
